...
export class Player {
  ...
  private getLookDirection(pointer: { x: number; y: number }) {
  ...
  private getAssetFromDirection(direction: DirectionsEnum) {
    switch (direction) {
  ...
  private handleAnimation(lookDirection: DirectionsEnum, isMoving: boolean) {
    if (!isMoving) {
